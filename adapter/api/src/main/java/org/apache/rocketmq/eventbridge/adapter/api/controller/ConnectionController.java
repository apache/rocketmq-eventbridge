        ...
        return Mono.subscriberContext()
            .map(ctx -> {
                ...
                List<String> errMessage = validate.stream()
                    ...
                    .collect(Collectors.toList());
                if (!CollectionUtils.isEmpty(errMessage)) {
                    return new GetConnectionResponse(null, null, null, null).parameterCheckFailRes(
                        errMessage.toString());
                }
                final ConnectionDTO connectionDTO = connectionService.getConnection(
                    accountAPI.getResourceOwnerAccountId(ctx), getConnectionRequest.getConnectionName());
                return new GetConnectionResponse(connectionDTO.getConnectionName(), connectionDTO.getDescription(),
                    connectionDTO.getNetworkParameters(), connectionDTO.getAuthParameters()).success();
            });
    }
